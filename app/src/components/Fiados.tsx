  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="flex flex-col gap-1">
        <span className="text-xs uppercase tracking-widest font-mono text-amber-500 font-bold block mb-1">
          Contas a Receber
        </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm flex flex-col justify-between items-start hover:shadow-md transition">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black font-mono text-slate-200">
              {fmt(totalEmAberto)}
            </h3>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm flex flex-col justify-between items-start hover:shadow-md transition">
          ...
          <div>
            <p className="text-xs text-amber-500/80 font-bold uppercase mb-1">
              Pendências
            </p>
        ...
        <div className="bg-rose-950/40 border border-rose-800/60 rounded-2xl p-5 shadow-sm flex flex-col justify-between items-start hover:shadow-md transition">
          ...
          <div>
            <p className="text-xs text-rose-500/80 font-bold uppercase mb-1">
              Dívidas Vencidas
            </p>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredFiados.map((f) => {
                ...
                return (
                  <tr
                    key={f.id}
                    className={`hover:bg-slate-800/50/80 transition ${isOverdue ? 'bg-rose-950/40/20' : ''}`}
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <span
                        className={`font-mono text-sm font-black ${isQuitado ? 'text-slate-500' : isOverdue ? 'text-rose-500' : 'text-amber-500'}`}
                      >
      ...
      {activeModal === 'quitar' && selectedFiado && (
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center p-4 z-50 animate-fade-in backdrop-blur-xs">
          <div className="w-full max-w-sm bg-slate-900 border border-emerald-800/60 rounded-2xl p-6 shadow-2xl relative">
            ...
            <p className="text-[10px] font-mono text-emerald-500 mb-4 block uppercase font-bold">
            ...
            <form onSubmit={handleQuitar} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full py-3 bg-emerald-600 hover:bg-emerald-950/400 text-white font-black rounded-xl text-xs transition cursor-pointer mt-2 shadow-md"
              >
                COMFIRMAR PAGAMENTO
              </button>
            </form>
