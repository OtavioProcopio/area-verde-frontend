...
export default function Produtos({
  ...
  const [activeTab, setActiveTab] = useState<'produtos' | 'categorias' | 'composicao'>('produtos');
  const [compositionProductId, setCompositionProductId] = useState<string | null>(null);
  ...
  const [filterStatus, setFilterStatus] = useState<'Tudo' | 'Ativos' | 'Inativos'>('Tudo');
  const [filterType, setFilterType] = useState<'Tudo' | 'Simples' | 'Composto'>('Tudo');
  ...
  const handleAddRecipeIngredient = () => {
    ...
    if (recipe.some(item => item.ingredientId === selectedIngredientId)) {
      ...
      setRecipe([...recipe, { ingredientId: selectedIngredientId, quantity: parsedQty }]);
    }
  ...
  const handleSaveComp = (e: React.FormEvent) => {
    ...
    if (compEditIndex !== null) {
       if (updatedRecipe.some((r, i) => i !== compEditIndex && r.ingredientId === compIngrId)) {
          alert('Este componente já está na receita!');
          return;
       }
       updatedRecipe[compEditIndex] = { ingredientId: compIngrId, quantity: parsedQty };
    } else {
  ...
  return (
    <div id="produtos-module" className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <span className="text-xs uppercase tracking-widest font-mono text-emerald-500 font-semibold block mb-1">Catálogo de Vendas</span>
          <h2 className="text-3xl font-display font-bold text-slate-50 tracking-tight animate-fade-in">
            Estoque & <span className="text-emerald-500 font-bold">Produtos</span>
          </h2>
        </div>
              ...
              <table className="w-full text-left border-collapse leading-normal text-xs text-slate-300">
                ...
                <tbody className="divide-y divide-slate-100 bg-slate-900">
                  {filteredProducts.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className={`transition duration-150 ${!pActive ? 'bg-slate-800/50/50 opacity-60 grayscale-[30%]' : 'hover:bg-slate-800/50/80'}`}>
                        <td className="px-5 py-3 align-middle max-w-[200px]">
                          <div>
                            <span className="font-bold text-slate-200 block text-xs truncate" title={p.name}>{p.name}</span>
                        ...
                        <td className="px-3 py-3 text-center font-mono align-middle">
                          <span className="text-[10px] font-bold text-slate-500">{p.unit.toUpperCase()}</span>
                        </td>
                        
                        <td className="px-3 py-3 text-center align-middle">
                          {p.isComposite ? (
                            <span className="px-2 py-0.5 bg-blue-950/40 border border-blue-800/60 rounded text-[9px] font-bold font-mono text-blue-400">COMP</span>
                          ) : (
                            <span className="px-2 py-0.5 bg-slate-800 border border-slate-800 rounded text-[9px] font-bold font-mono text-slate-500">SIMPLES</span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3 text-center align-middle">
                          <button 
                            ...
                            className={`inline-flex items-center gap-1 font-mono text-[9px] uppercase font-bold px-2 py-1 rounded transition border cursor-pointer ${
                              pActive ? 'bg-emerald-950/40 border-emerald-800/60 text-emerald-400 hover:bg-emerald-900/50 hover:border-emerald-300' : 'bg-slate-800 border-slate-800 text-slate-500 hover:bg-slate-700'
                            }`}
                          >
                           {pActive ? <CheckCircle2 size={10} /> : <XCircle size={10} />}
                          ...
                          <div className="flex items-center justify-end gap-1.5 flex-nowrap">
                            ...
                            {!p.isComposite && pActive && (
                              <div className="flex items-center bg-slate-800/50 p-0.5 border border-slate-800 rounded-lg">
                                ...
                                />
                                <button
                                  onClick={() => handleQuickRestock(p.id, parseFloat(quickRestockAmount) || 12)}
                  ...
                  {filteredProducts.length === 0 && (
                    <tr>
                      <td colSpan={7} className="text-center py-12 text-slate-500 italic text-xs bg-slate-800/50/30">
        ...
        <div className="space-y-4 animate-fade-in">
           <div className="flex justify-between items-center bg-slate-900 p-4 rounded-xl border border-slate-800 shadow-sm">
             <div className="flex-1">
               <h3 className="font-display font-bold text-slate-200">Categorias</h3>
               <p className="text-xs text-slate-500">Classificação para filtros e PDV</p>
             </div>
             <button
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {categories.map((c) => (
                 <div key={c.id} className={`bg-slate-900 rounded-xl border p-4 shadow-2xs flex flex-col justify-between transition ${c.active ? 'border-slate-800' : 'border-slate-800 bg-slate-800/50/50 opacity-60'}`}>
                    <div className="flex justify-between items-start mb-4">
                       <div className="flex items-center gap-2 text-slate-200">
                          <Tags size={16} className={c.active ? 'text-emerald-500' : 'text-slate-500'} />
                          <h4 className="font-bold font-display">{c.name}</h4>
                       </div>
                       <button onClick={() => handleToggleCatStatus(c)} className={`cursor-pointer ${c.active ? 'text-emerald-500 hover:text-emerald-500' : 'text-slate-500 hover:text-slate-500'}`}>
      ...
      {activeTab === 'composicao' && activeComposite && (
         <div className="space-y-4 animate-fade-in pb-10">
            <div className="flex justify-between items-center bg-slate-900 p-4 rounded-xl border border-blue-800/60 shadow-sm flex-wrap gap-2">
               <div>
                  <button onClick={() => setActiveTab('produtos')} className="text-[10px] uppercase font-mono font-bold text-slate-500 hover:text-slate-200 transition block mb-1 cursor-pointer">
                     &larr; Voltar
                  </button>
            ...
            <div className="bg-blue-950/40/50 border border-blue-900 rounded-xl p-4 text-xs text-blue-800 shadow-sm flex items-start gap-4 mx-1">
               ...
               <div>
                  <h4 className="font-bold mb-1 font-display">Como funciona a composição da receita?</h4>
                  <p className="text-blue-400/80 leading-relaxed font-mono mt-2">
                     Ao vender <strong className="text-blue-900 border-b border-blue-300 pb-0.5">1 {activeComposite.unit}</strong> de <strong className="text-blue-900 border-b border-blue-300 pb-0.5">{activeComposite.name}</strong>, 
                     o sistema descontará automaticamente do estoque físico as quantidades exatas dos insumos configurados abaixo.
                  </p>
               </div>
            </div>
            ...
            {(!activeComposite.recipe || activeComposite.recipe.length === 0) ? (
               <div className="text-center bg-slate-900 border border-slate-800 rounded-xl p-12 shadow-sm text-slate-500 mt-2 mx-1">
                  ...
                  <h5 className="font-bold text-slate-300 mb-2">Composição Vazia</h5>
                  <p className="text-xs max-w-sm mx-auto">Este produto composto não tem componentes. Se ele for vendido agora, nenhum estoque extra será baixado. Adicione componentes para ligar o controle de estoque.</p>
               </div>
            ) : (
                     ...
                     <table className="w-full text-left border-collapse leading-normal text-xs text-slate-300">
                       ...
                       <tbody className="divide-y divide-slate-100 bg-slate-900">
                         {activeComposite.recipe.map((r, idx) => {
                            const ingr = products.find(p => p.id === r.ingredientId);
                            return (
                              <tr key={idx} className="hover:bg-slate-800/50/50 transition duration-150">
                                 <td className="px-5 py-3">
                                    <span className="font-bold text-slate-200">{ingr ? ingr.name : 'Desconhecido'}</span>
                                 </td>
                                 ...
                                 <td className="px-4 py-3 text-center">
                                    {ingr ? (
                                       (ingr.active ?? true) ? (
                                          <span className="px-2 py-0.5 rounded text-[9px] uppercase font-bold font-mono bg-emerald-950/40 text-emerald-400 border border-emerald-800/60 shadow-2xs">ATIVO</span>
                                       ) : (
                                          <span className="px-2 py-0.5 rounded text-[9px] uppercase font-bold font-mono bg-rose-950/40 text-rose-400 border border-rose-800/60 shadow-2xs">INATIVO</span>
                                       )
                                    ) : '--'}
                                 </td>
      ...
      {showCompModal && (
         <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center p-4 z-50 animate-fade-in backdrop-blur-xs">
            <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl relative">
              <button type="button" onClick={() => setShowCompModal(false)} className="absolute top-4 right-4 text-slate-500 hover:text-slate-200 cursor-pointer">
                <X size={18} />
              </button>
              <h3 className="text-lg font-display font-bold text-slate-200 mb-4">{compEditIndex !== null ? 'Editar Componente' : 'Adicionar Componente'}</h3>
              
              <form onSubmit={handleSaveComp} className="space-y-4">
                 <div>
                    <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1">Insumo Físico <span className="text-rose-500">*</span></label>
                    <select
                      value={compIngrId}
                    ...
                    >
                       ...
                       {products.filter(p => !p.isComposite && (p.active ?? true)).map(p => (
                          <option key={p.id} value={p.id}>{p.name}</option>
                       ))}
                    </select>
                    <span className="text-[9px] text-slate-500 block mt-1">Apenas produtos simples e ativos.</span>
                 </div>
                 ...
                 <div>
                    ...
                    <div className="flex relative">
                       ...
                       <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] uppercase font-mono font-bold text-slate-500">
                          {compIngrId ? (products.find(p => p.id === compIngrId)?.unit || 'UN') : 'UN'}
                       </span>
                    </div>
                 </div>

                 <button type="submit" className="w-full py-2.5 bg-blue-600 hover:bg-blue-950/400 text-white font-bold rounded-xl text-xs transition cursor-pointer">
                   Salvar Componente
                 </button>
              </form>
            </div>
      ...
      {showCatForm && (
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center p-4 z-50 animate-fade-in backdrop-blur-xs">
           <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl relative">
             <button onClick={() => setShowCatForm(false)} className="absolute top-4 right-4 text-slate-500 hover:text-slate-200 cursor-pointer">
               <X size={18} />
             </button>
             <h3 className="text-lg font-display font-bold text-slate-200 mb-4">{catFormMode === 'create' ? 'Nova Categoria' : 'Renomear Categoria'}</h3>
             
             <form onSubmit={handleSaveCat} className="space-y-4">
                <div>
                   <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1">Nome:</label>
                   <input
                     type="text"
                     value={catName}
            ...
            <div className="flex justify-between items-center mb-4 shrink-0">
               <div>
                 <h3 className="text-xl font-display font-bold text-slate-850 flex items-center gap-1.5">
                   ...
                   {formMode === 'create' ? 'Cadastrar Novo Item' : 'Editar Ficha do Produto'}
                 </h3>
                 <p className="text-xs text-slate-500 font-mono">Controle preciso de custo, venda e estoque seguro do balcão.</p>
               </div>
               <button onClick={() => setShowProductForm(false)} className="text-slate-500 hover:text-slate-200 cursor-pointer p-2 bg-slate-800/50 rounded-lg">
                 <X size={18} />
               </button>
            </div>

            <form onSubmit={handleSaveProduct} className="flex-1 overflow-y-auto pr-2 space-y-6 form-scrollbar">
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {/* Basic fields */}
                <div className="space-y-3">
                  <div>
                    <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1">Nome do Produto <span className="text-rose-500">*</span></label>
                    <input
                      type="text"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1">Categoria <span className="text-rose-500">*</span></label>
                      <select
                    ...
                    <div>
                      <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1">Status</label>
                      <select
                  ...
                  <div className="grid grid-cols-2 gap-3 pb-3 border-b border-slate-800">
                    <div>
                      <label className="block text-[10px] uppercase font-mono text-slate-500 mb-1">Preço Custo (R$)</label>
                      <input
                        type="number"
                        step="0.01"
                        min="0"
                    ...
                    <div>
                      <label className="block text-[10px] uppercase font-mono text-emerald-300 mb-1 font-bold">Venda (R$) <span className="text-rose-500">*</span></label>
                      <input
                        type="number"
                        step="0.01"
                        min="0"
                  ...
                  <div className={`flex items-start gap-2.5 p-3 border rounded-xl transition ${isComposite ? 'bg-blue-950/40 border-blue-800/60' : 'bg-slate-800/50 border-slate-800'}`}>
                    <input
                    ...
                    />
                    <label htmlFor="checkbox-is-composite" className="text-xs text-slate-300 cursor-pointer">
                      <span className="font-bold block text-slate-50 mb-0.5">Produto Composto? (Receita)</span>
                      Ex: Caipirinha. A venda desconta estoque proporcionalmente de insumos.
                    </label>
                  </div>
                  ...
                  {!isComposite && (
                    <div className="grid grid-cols-3 gap-3 p-3 bg-slate-900 shadow-xs rounded-xl border border-slate-800">
                      <div>
                        <label className="block text-[9px] uppercase font-mono text-slate-500 mb-1">Unidade</label>
                        <select
                          ...
                          value={unit}
                          onChange={(e) => setUnit(e.target.value as 'un' | 'ml')}
                        >
                      ...
                      <div>
                        <label className="block text-[9px] uppercase font-mono text-slate-500 mb-1">Estoque</label>
                        <input
                          type="number"
                          step="0.01"
                      ...
                      <div>
                        <label className="block text-[9px] uppercase font-mono text-slate-500 mb-1">Alerta Mín</label>
                        <input
                          type="number"
                          step="1"
                          className="w-full bg-slate-800/50 border border-slate-800 px-2 py-1.5 text-xs font-mono text-slate-200 rounded-lg outline-none focus:border-emerald-600"
                  ...
                  {isComposite && (
                     <div className="grid grid-cols-1 gap-3 p-3 bg-slate-800/50 rounded-xl border border-slate-800">
                      <div className="text-[11px] text-slate-400 leading-relaxed">
                        Produto composto na API não possui unidade de estoque própria. A baixa acontece pelos componentes cadastrados na composição.
                      </div>
                     </div>
                  )}
                </div>
                ...
                <div className="flex flex-col">
                  {isComposite ? (
                    <div className="rounded-2xl border-2 border-dashed border-slate-800 h-full flex flex-col justify-center items-center text-center p-6 text-slate-500 bg-blue-950/40/10">
                      ...
                      <h5 className="text-sm font-bold text-slate-500">Produto Composto</h5>
                      <p className="text-[10px] text-slate-500 mt-2 max-w-[200px] leading-relaxed">
                        Este produto é formado por outros insumos. Para incluí-los ou removê-los, volte para a lista de produtos, encontre-o e clique no botão <span className="font-bold text-blue-500">azul de composição</span>.
                      </p>
                    </div>
                  ) : (
                    <div className="rounded-2xl border-2 border-dashed border-slate-800 h-full flex flex-col justify-center items-center text-center p-6 text-slate-500 bg-slate-800/50/50">
                      ...
                      <h5 className="text-sm font-bold text-slate-500">Configuração de Físico</h5>
                      <p className="text-[10px] text-slate-500 mt-2 max-w-[200px] leading-relaxed">
                        Este produto deduzirá o estoque direto dele mesmo na hora venda.
                      </p>
                    </div>
                  )}
                </div>
