...
interface ComandasProps {
  ...
  onUpdateComanda: (id: string, updates: Partial<Comanda>) => void | Promise<void>;
  onAddItemToComanda: (comandaId: string, item: Omit<TabItem, 'id'>) => void | Promise<void>;
  onUpdateComandaItemQty: (comandaId: string, itemId: string, qty: number) => void | Promise<void>;
  onRemoveItemFromComanda: (comandaId: string, itemId: string) => void | Promise<void>;
  ...
  onAddCustomer: (c: { name: string; phone: string; notes?: string }) => void | Promise<void>;
}
...
export default function Comandas({
  ...
  const [listStatusFilter, setListStatusFilter] = useState<'all' | 'active' | 'paid' | 'cancelled' | 'fiado'>('all');
  ...
  const [checkoutPaymentMethod, setCheckoutPaymentMethod] = useState<'dinheiro' | 'pix' | 'cartao' | 'fiado'>('dinheiro');
  ...
  const filteredProducts = products.filter((prod) => {
    ...
    const matchesCat = prod.category === selectedCategory || selectedCategory === 'Tudo';
    return matchesSearch && matchesCat;
  });

  // Calculate totals of selected comanda
  const subtotalSelected = selectedComanda
    ? selectedComanda.items.reduce((sum, item) => sum + (item.price * item.quantity), 0)
    : 0;
  ...
  const handleCreateComanda = async (e: React.FormEvent) => {
    ...
    const mappedCustId = newComandaCustomerId === '' ? null : newComandaCustomerId;
  ...
  const handleAddProductToComanda = (p: Product) => {
    ...
    if (!p.isComposite && p.stock <= 0) {
      if (!window.confirm(`Atenção: '${p.name}' está com estoque zerado! Deseja registrar a saída mesmo assim?`)) {
        return;
      }
    }
  ...
  const handleLaunchCheckout = () => {
    ...
    if (!caixaIsOpen) {
      alert('Atenção: O caixa diário está FECHADO! Abra o caixa primeiro na aba "Controle de Caixa".');
      return;
    }
  ...
  const handleConfirmCheckout = async () => {
    ...
    if (res.success) {
      ...
      const completedComanda: Comanda = {
        ...
        customerId: checkoutPaymentMethod === 'fiado' ? checkoutCustomerId : selectedComanda.customerId
      };
      ...
      const remaining = comandas.filter(c => c.status === 'active' && c.id !== selectedComanda.id);
  ...
  const filteredListComandas = comandas.filter((c) => {
    const term = listSearchTerm.toLowerCase();
    const cust = customers.find(cu => cu.id === c.customerId);
    const matchesSearch = c.code.toLowerCase().includes(term) || c.id.toLowerCase().includes(term) || (cust && cust.name.toLowerCase().includes(term));
    if (listSearchTerm && !matchesSearch) return false;

    if (listStatusFilter === 'active' && c.status !== 'active') return false;
    if (listStatusFilter === 'paid' && (c.status !== 'paid' || c.paymentMethod === 'fiado')) return false;
    if (listStatusFilter === 'cancelled' && c.status !== 'cancelled') return false;
    if (listStatusFilter === 'fiado' && c.paymentMethod !== 'fiado') return false;
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Visão geral de tickets ativos, fechados e pendências de fiado.</p>
            </div>
            <button
              ...
              <table className="w-full text-left text-xs text-slate-500 border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredListComandas.map(c => {
                    ...
                    const bindedCust = customers.find(cu => cu.id === c.customerId);
                    
                    let pago = 0;
                    let restante = 0;
                    if (c.status === 'paid' && c.paymentMethod !== 'fiado') {
                      pago = totalCom;
                    } else if (c.status === 'paid' && c.paymentMethod === 'fiado') {
                      pago = 0;
                      restante = totalCom;
                    ...
                    const isFiado = c.status === 'paid' && c.paymentMethod === 'fiado';

                    return (
                      <tr key={c.id} className={`hover:bg-slate-800/50 transition duration-150 ${isAberta ? 'bg-emerald-950/40/20' : ''}`}>
                        <td className="px-4 py-3.5">
                          <div className="font-bold text-slate-200 text-sm truncate max-w-[150px]">{c.code}</div>
                          <div className="text-[9px] text-slate-500 font-mono mt-0.5" title={new Date(c.createdAt).toLocaleString('pt-BR')}>
                        ...
                        <td className="px-4 py-3.5">
                          {bindedCust ? (
                            ...
                            <span className="text-slate-500 italic">Avulso</span>
                          )}
                        </td>
                        <td className="px-4 py-3.5">
                          {c.status === 'active' && <span className="inline-flex text-[10px] uppercase font-bold font-mono px-2 py-0.5 rounded-full bg-amber-900/50 text-amber-400 border border-amber-800/60">ABERTA</span>}
                          {c.status === 'cancelled' && <span className="inline-flex text-[10px] uppercase font-bold font-mono px-2 py-0.5 rounded-full bg-slate-800 text-slate-500 border border-slate-800">CANCELADA</span>}
                          {isFiado && <span className="inline-flex text-[10px] uppercase font-bold font-mono px-2 py-0.5 rounded-full bg-rose-900/50 text-rose-400 border border-rose-800/60">PENDENTE (FIADO)</span>}
                          {c.status === 'paid' && !isFiado && <span className="inline-flex text-[10px] uppercase font-bold font-mono px-2 py-0.5 rounded-full bg-emerald-900/50 text-emerald-400 border border-emerald-800/60">PAGA</span>}
                        </td>
                        <td className="px-4 py-3.5 font-mono text-right font-bold text-slate-200">{fmt(totalCom)}</td>
                        <td className="px-4 py-3.5 font-mono text-right text-emerald-500 font-bold">{fmt(pago)}</td>
                        <td className="px-4 py-3.5 font-mono text-right text-rose-500 font-bold">{restante > 0 ? fmt(restante) : '-'}</td>
                        <td className="px-4 py-3.5 text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            {c.status === 'active' && (
                              <button
                                onClick={() => {
                                  if (window.confirm(`Tem certeza que deseja cancelar a comanda ${c.code}?`)) {
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            {!activeComanda ? (
              <div className="flex-1 flex flex-col justify-center items-center text-center p-12 bg-slate-900 rounded-xl border border-slate-800 text-slate-500">
                ...
                <h5 className="font-display font-bold text-slate-300 mb-1">Nenhuma comanda em atendimento</h5>
                <p className="text-xs max-w-xs text-slate-500">Abra uma nova ou selecione ao lado.</p>
              </div>
            ) : (() => {
              const comSum = activeComanda.items.reduce((sum, item) => sum + (item.price * item.quantity), 0);
              const totalCom = Math.max(0, comSum);
              const bindedCust = customers.find(cu => cu.id === activeComanda.customerId);
              
              let pago = 0;
              let restante = 0;
              if (activeComanda.status === 'paid' && activeComanda.paymentMethod !== 'fiado') {
                pago = totalCom;
              } else if (activeComanda.status === 'paid' && activeComanda.paymentMethod === 'fiado') {
                pago = 0;
                restante = totalCom;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start min-h-[600px] h-auto lg:h-[calc(100vh-140px)]">
                  ...
                  <div className="col-span-1 lg:col-span-6 xl:col-span-7 bg-slate-900 rounded-xl border border-slate-800 shadow-sm flex flex-col h-full overflow-hidden">
                    ...
                    <div className="bg-slate-800 text-white p-4 flex justify-between items-center shrink-0">
                       <div>
                          <div className="flex items-center gap-2 mb-1">
                             <h2 className="text-2xl font-display font-black tracking-tight">{activeComanda.code}</h2>
                             <span className="text-[10px] uppercase font-mono font-bold bg-amber-950/400 text-slate-50 px-1.5 py-0.5 rounded">
                                {activeComanda.status === 'active' ? 'ABERTA' : (activeComanda.status === 'paid' && activeComanda.paymentMethod === 'fiado' ? 'FIADO' : (activeComanda.status === 'paid' ? 'PAGOU' : 'CANCELADA'))}
                             </span>
                          </div>
                          <div className="text-xs text-slate-300 flex items-center gap-1.5 font-mono">
                             <UserCheck size={12} />
                             {bindedCust ? (
                               <span className="font-bold text-emerald-300">Cliente: {bindedCust.name}</span>
                             ) : (
                             ...
                             <div className="relative ml-2 flex items-center">
                               <select 
                                 ...
                                 onChange={(e) => onUpdateComanda(activeComanda.id, { customerId: e.target.value || null })}
                               >
                       ...
                       <div className="text-right flex flex-col items-end">
                          {activeComanda.status === 'active' && (
                            <button
                              onClick={() => {
                                if (window.confirm(`Cancelar definitivamente a comanda ${activeComanda.code}?`)) {
                    ...
                    <div className="flex-1 overflow-y-auto p-4 bg-slate-800/50 space-y-2">
                       {activeComanda.items.map(item => {
                         ...
                         return (
                           <div key={item.id} className="bg-slate-900 border border-slate-800 rounded-lg p-3 flex shadow-2xs items-center gap-3">
                              <div className="flex-1 min-w-0">
                                 <div className="flex items-center gap-2">
                                    <h4 className="text-sm font-bold text-slate-200 truncate">{item.productName}</h4>
                                    {product?.isComposite ? (
                                      <span className="text-[9px] bg-blue-950/40 text-blue-400 font-mono font-bold px-1.5 py-0.5 rounded border border-blue-800/60">COMPOSTO</span>
                                    ) : (
                                      <span className="text-[9px] bg-slate-800 text-slate-500 font-mono font-bold px-1.5 py-0.5 rounded border border-slate-800">SIMPLES</span>
                                    )}
                                 </div>
                                 <div className="text-[11px] text-slate-500 font-mono mt-0.5">{fmt(item.price)} un.</div>
                              </div>
                              ...
                              <div className="flex items-center bg-slate-800/50 border border-slate-800 rounded-lg shrink-0">
                                <button
                                  onClick={() => {
                                    if(item.quantity === 1) {
                                       if(window.confirm(`Remover ${item.productName}?`)) onRemoveItemFromComanda(activeComanda.id, item.id);
                                    } else {
                                       onUpdateComandaItemQty(activeComanda.id, item.id, item.quantity - 1);
                                    }
                                  }}
                                ...
                                <span className="w-8 text-center text-xs font-bold text-slate-300 font-mono">{item.quantity}</span>
                                <button
                                  onClick={() => onUpdateComandaItemQty(activeComanda.id, item.id, item.quantity + 1)}
                              ...
                              <div className="text-right min-w-[70px] shrink-0">
                                 <div className="text-sm font-black text-slate-200 font-mono">{fmt(item.price * item.quantity)}</div>
                              </div>

                              <button
                                onClick={() => {
                                  if(window.confirm('Excluir item inteiramente?')) onRemoveItemFromComanda(activeComanda.id, item.id);
                                }}
                       ...
                       {activeComanda.items.length === 0 && (
                         <div className="text-center py-10 text-slate-500 text-xs italic flex flex-col items-center">
                           ...
                           Nenhum item lançado.<br/>Busque e adicione no painel lateral.
                         </div>
                       )}
                    </div>
                    ...
                    <div className="bg-slate-900 border-t border-slate-800 p-4 shrink-0 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
                       ...
                       <div className="text-xs font-mono text-slate-400 mb-3 bg-slate-800/50 border border-slate-800 rounded-lg px-3 py-2">
                          Fechamento pela API usa o total exato dos itens. Desconto, acréscimo e taxa de serviço não estão disponíveis neste fluxo.
                       </div>
                       
                       {/* Totals Box Horizontal */}
                       <div className="bg-slate-800 rounded-xl p-4 flex justify-between items-center mb-3 text-white">
                         <div className="text-center">
                            <span className="text-[9px] uppercase font-bold text-slate-500 block mb-0.5 tracking-widest">Valor Recebido</span>
                            <span className="text-lg font-mono font-bold text-emerald-400">{fmt(pago)}</span>
                         </div>
                         <div className="w-px h-10 bg-slate-600/50"></div>
                         <div className="text-center">
                            <span className="text-[9px] uppercase font-bold text-slate-500 block mb-0.5 tracking-widest">Módulo Restante</span>
                            <span className="text-lg font-display font-black text-rose-400">{fmt(restante)}</span>
                         </div>
                         <div className="w-px h-10 bg-slate-600/50"></div>
                         <div className="text-right">
                            <span className="text-[10px] uppercase font-bold text-slate-500 block mb-0.5 tracking-widest">Total da Conta</span>
                            <span className="text-3xl font-mono font-black text-white">{fmt(totalCom)}</span>
                         </div>
                       </div>
                       ...
                       {activeComanda.status === 'active' && (
                         <div className="flex gap-2">
                           ...
                           >
                              ...
                              <span className="text-[11px] font-normal text-emerald-100">Dinheiro, Pix ou Cartão</span>
                           </button>
                         </div>
                       )}
                    </div>
                  ...
                  <div className="col-span-1 lg:col-span-6 xl:col-span-5 bg-slate-900 rounded-xl border border-slate-800 shadow-sm flex flex-col h-full overflow-hidden p-4">
                     ...
                     <div className="relative mb-3 shrink-0">
                        <Search size={20} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-emerald-500" />
                        <input
                          type="text"
                          placeholder="Ex: Skol"
                          value={searchTerm}
                     ...
                     <div className="flex gap-1.5 overflow-x-auto pb-2 shrink-0 mb-3 scrollbar-hide">
                        {['Tudo', ...categories.map(c => c.name).filter(n => n !== 'Ingredientes')].map((cat) => (
                          <button
                            key={cat}
                     ...
                     <div className="flex-1 overflow-y-auto grid grid-cols-2 gap-2 pr-1 content-start">
                        {filteredProducts.map(p => {
                           ...
                           return (
                             <button
                               key={p.id}
                               onClick={() => !outOfStock && handleAddProductToComanda(p)}
                               disabled={outOfStock}
                               className={`p-3 bg-slate-900 border border-slate-800 rounded-xl text-left flex flex-col justify-between h-[90px] shadow-sm transition ${
                                 outOfStock ? 'opacity-40 cursor-not-allowed' : 'hover:border-emerald-500 hover:shadow-md cursor-pointer'
                               }`}
                             >
                               <div>
                                  <span className="text-[9px] uppercase text-slate-500 font-mono font-bold block mb-0.5 truncate">{p.category}</span>
                                  <span className="text-xs font-bold text-slate-200 leading-tight line-clamp-2">{p.name}</span>
                               </div>
                               <div className="flex justify-between items-end w-full">
                                  <span className="text-[13px] font-mono font-black text-emerald-400">{fmt(p.price)}</span>
                                  <span className="text-[9px] text-slate-500 font-mono font-bold bg-slate-800 px-1 rounded">{p.isComposite ? 'CMP' : `${p.stock} ${p.unit.toUpperCase()}`}</span>
                               </div>
                             </button>
                           );
                        })}
                     ...
                     <div className="mt-4 border-t border-slate-800 pt-4 shrink-0 bg-slate-800/50/50 -mx-4 -mb-4 p-4 rounded-b-xl border-dashed">
                         <span className="text-[10px] text-slate-500 uppercase font-mono font-bold block mb-2 flex items-center gap-1"><Plus size={12}/> Lançamentos avulsos</span>
                         <p className="text-xs text-slate-400 leading-relaxed">
                           A API atual só permite adicionar itens previamente cadastrados em produtos. Para venda eventual, cadastre o item no catálogo e lance por aqui.
                         </p>
                     </div>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center p-4 z-50 animate-fade-in backdrop-blur-xs">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl relative">
            ...
            <form onSubmit={handleCreateComanda} className="space-y-4">
              <div>
                <label className="block text-xs uppercase font-mono text-slate-505 mb-1 font-bold">Identificação / Nome (Ex: Comanda 15, Balcão João):</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs uppercase font-mono text-slate-505 mb-1 font-bold">Vincular Cliente Registrado (Opcional):</label>
                <select
                ...
                >
                  ...
                  {customers.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.name} {c.balance > 0 ? `(Dívida: ${fmt(c.balance)})` : ''}
                    </option>
                  ))}
                </select>
              </div>
      ...
      {showCheckoutModal && selectedComanda && (
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center p-4 z-50 animate-fade-in backdrop-blur-xs">
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl relative">
            ...
            <h3 className="text-lg font-display font-bold text-slate-850 mb-1">Finalização Balcão</h3>
            <p className="text-xs text-slate-500 font-mono mb-4">Comanda: {selectedComanda.code}</p>

            <div className="grid grid-cols-1 md:grid-cols-12 gap-5 mb-5 border-y border-slate-700 py-4">
              
              {/* Payment selection */}
              <div className="md:col-span-6 space-y-2.5">
                <label className="block text-xs uppercase font-mono text-slate-500 font-bold">Forma de Pagamento:</label>
              ...
              <div className="md:col-span-6 flex flex-col justify-between">
                <div>
                  <span className="text-xs uppercase font-mono text-slate-500 block mb-2 font-bold">Resumo da Conta:</span>
                  <div className="space-y-1 bg-slate-800/50 p-3 rounded-lg border border-slate-800 text-xs font-mono">
                    ...
                    <div className="flex justify-between text-slate-200 font-bold border-t border-slate-800 pt-1.5 mt-1.5 text-sm">
                      <span>Total:</span>
                      <span className="text-emerald-400">{fmt(totalSelected)}</span>
                    </div>
                  </div>
                </div>
                ...
                {checkoutPaymentMethod === 'fiado' && (
                  <div className="mt-4 space-y-2">
                    <div className="flex justify-between items-center">
                      <label className="block text-[10px] uppercase font-mono text-rose-750 font-bold">Vincular Cliente do Caderno:</label>
                      <button
                        type="button"
                    ...
                    {!showNewClientForm ? (
                      ...
                      <form onSubmit={handleQuickAddClient} className="bg-slate-800/50 p-2.5 rounded border border-slate-800 space-y-1.5 text-left">
                        <input
                          type="text"
                        ...
                        <input
                          ...
                          className="w-full bg-slate-900 border border-slate-250 py-1 px-1.5 text-[10px] rounded text-slate-200"
                        />
                        <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-950/400 font-bold text-[10px] rounded text-white cursor-pointer py-1">Salvar</button>
                      </form>
                    )}
                  </div>
                )}
          ...
          <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl relative">
            ...
            <div className="bg-stone-50 text-stone-900 font-mono text-[11px] p-5 rounded-lg border border-slate-250 space-y-3.5 shadow-inner leading-normal">
              <div className="text-center font-bold">
                <p className="text-sm tracking-tight font-sans font-extrabold text-slate-850">BAR AREA VERDE</p>
                <p className="text-[9px] font-normal tracking-wide">AMIGOS E FAMILIA LTDA</p>
                <p className="text-[8px] font-normal text-stone-500 border-b border-dashed border-stone-300 pb-2">AV. DA AMIZADE, 1800 - BALCÃO</p>
              </div>

              <div>
                <p className="font-bold uppercase tracking-wider text-center text-[10px] text-stone-605">** CUPOM OPERACIONAL **</p>
                <div className="grid grid-cols-2 mt-2 gap-y-0.5 text-[9px] text-stone-650">
                  <span>Comanda/ID:</span>
                  <span className="text-right font-bold">{receiptComanda.code}</span>
                  <span>Data:</span>
                  <span className="text-right">{new Date(receiptComanda.paidAt || '').toLocaleDateString('pt-BR')}</span>
                  <span>Hora:</span>
                  <span className="text-right">{new Date(receiptComanda.paidAt || '').toLocaleTimeString('pt-BR')}</span>
                  <span>Pagamento:</span>
                  <span className="text-right font-bold text-emerald-300">{(receiptComanda.paymentMethod || 'DINHEIRO').toUpperCase()}</span>
                </div>
              </div>
              ...
              <div className="border-t border-b border-dashed border-stone-300 py-2 space-y-1 select-none text-[9px]">
                <div className="grid grid-cols-12 font-bold text-stone-500 mb-1">
                  <span className="col-span-6 animate-pulse">PRODUCT DESCRIPTION</span>
                ...
                {receiptComanda.items.map((item) => (
                  <div key={item.id} className="grid grid-cols-12 text-stone-800">
                    ...
                    <span className="col-span-2 text-right font-medium">{item.quantity}</span>
                    <span className="col-span-4 text-right font-bold font-mono">{(item.price * item.quantity).toFixed(2)}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-0.5 text-right text-[10px] text-stone-700">
                <p className="flex justify-between">
                  <span>SUBTOTAL:</span>
                  <span>{receiptComanda.items.reduce((sum, item) => sum + (item.price * item.quantity), 0).toFixed(2)}</span>
                </p>
                ...
                <p className="flex justify-between font-bold text-xs pt-1.5 border-t border-dashed border-stone-300 mt-1 text-slate-50">
                  ...
                  <span className="text-sm font-sans font-extrabold text-emerald-805">
                    R$ { (receiptComanda.items.reduce((sum, item) => sum + (item.price * item.quantity), 0) - receiptComanda.discount + receiptComanda.addition).toFixed(2) }
                  </span>
                </p>
              </div>
              ...
              <div className="text-center text-[8px] text-stone-450 pt-3 border-t border-dashed border-stone-300 space-y-0.5">
                ...
                <p className="font-mono mt-1 text-[7px] text-stone-400">Sistema Area Verde - POS Balcão</p>
              </div>
            </div>
